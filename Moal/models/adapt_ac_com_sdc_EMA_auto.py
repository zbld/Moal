...
#         else:
#             if hasattr(self._network, "module"):
#                 self._network.module.update_fc(self._total_classes, self.args['Hidden'], cosine_fc=True)
#                 self._network.module.update_fc(self._total_classes, self.args['Hidden'])
#             else:
...
#             net_model = self._network.module if hasattr(self._network, "module") else self._network
#             for param in net_model.ac_model.parameters():
...
class Learner(BaseLearner):
    ...
    def _train(self, train_loader, test_loader, train_loader_for_protonet):
        ...
        if self._cur_task == 0:
            ...
            if total_params != total_trainable_params:
                for name, param in net.named_parameters():
                    if param.requires_grad:
                        print(name, param.numel())
            if self.args['optimizer'] == 'sgd':
                optimizer = optim.SGD(net.parameters(), momentum=0.9, lr=self.init_lr,
                                      weight_decay=self.weight_decay)
            elif self.args['optimizer'] == 'adam':
            ...
            print(f'{total_trainable_params:,} training parameters.')
            if self.args['optimizer'] == 'sgd':
                optimizer = optim.SGD(net.parameters(), momentum=0.9, lr=self.progressive_lr,
                                      weight_decay=self.weight_decay)
            elif self.args['optimizer'] == 'adam':
    ...
    def _init_train(self, train_loader, test_loader, optimizer, scheduler):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            test_acc = self._compute_accuracy(net, test_loader)
            info = "Task {}, Epoch {}/{} => Loss {:.3f}, Train_accy {:.2f}, Test_accy {:.2f}".format(
                self._cur_task,
                epoch + 1,
                self.args['tuned_epoch'],
    ...
    def _progreessive_train(self, train_loader, test_loader, optimizer, scheduler):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                inputs, targets = inputs.to(self._device), targets.to(self._device)
                logits = net(inputs)["train_logits"]
            ...
            for param, ema_param in zip(net.backbones[0].parameters(), EMA_model.backbones[0].parameters()):
                ema_param.data = alpha * ema_param.data + (1 - alpha) * param.data

            scheduler.step()
            train_acc = np.around(tensor2numpy(correct) * 100 / total, decimals=2)

            test_acc = self._compute_ac_train_accuracy(net, test_loader)
            info = "Task {}, Epoch {}/{} => Loss {:.3f}, Train_accy {:.2f}, Test_accy {:.2f}".format(
                self._cur_task,
                epoch + 1,
                self.args['progreesive_epoch'],
    ...
    def extract_prototype(self, loader, net=None):
        ...
        for _, _inputs, _targets in loader:
            _targets = _targets.numpy()
            _vectors = tensor2numpy(net(_inputs.to(self._device))["features"])
            vectors.append(_vectors)
            targets.append(_targets)
        return np.concatenate(vectors), np.concatenate(targets)
    ...
    def cali_prototye_model(self, train_loader):
        ...
        with torch.no_grad():
            ...
            for i, batch in pbar:
                ...
                images = data.to(self._device)
                old_feature = tensor2numpy(self.old_network_module_ptr(images)["features"])
                feature = tensor2numpy(net(images)["features"])
                old_vectors.append(old_feature)
                vectors.append(feature)
        E_old = np.concatenate(old_vectors)
        E_new = np.concatenate(vectors)
        ...
        train_dataloader = DataLoader(train_dataset, batch_size=32, shuffle=True)
        test_dataloader = DataLoader(test_dataset, batch_size=32, shuffle=False)
        in_features = E_old[0].shape[0]
        out_dim = E_new[0].shape[0]
        calimodel = SimpleNN(in_features, out_dim)
        calimodel = calimodel.to(self._device)
