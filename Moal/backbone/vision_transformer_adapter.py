...
def vit_base_patch16_224_adapter_dino(pretrained=False, **kwargs):
    ...
    # checkpoint_model = torch.load('./pretrained_models/B_16-i21k-300ep-lr_0.001-aug_medium1-wd_0.1-do_0.0-sd_0.0.npz')
    #checkpoint_model=timm.create_model("vit_base_patch16_224", pretrained=True, num_classes=0)
    #state_dict = torch.load('./checkpoints/dino_vitbase16_pretrain.pth', map_location='cpu')['state_dict']
    ckpt = torch.load('/root/autodl-tmp/jww/CVPR/ACIL-LAMDA-PILOT/checkpoints/dino_vitbase16_pretrain.pth', map_location='cpu')
    #state_dict = checkpoint_model.state_dict()
    # modify the checkpoint state dict to match the model
    # first, split qkv weight into q, k, v
    for key in list(ckpt.keys()):
        if 'qkv.weight' in key:
            qkv_weight = ckpt.pop(key)
            q_weight = qkv_weight[:768]
            k_weight = qkv_weight[768:768*2]
            v_weight = qkv_weight[768*2:]
            ckpt[key.replace('qkv.weight', 'q_proj.weight')] = q_weight
            ckpt[key.replace('qkv.weight', 'k_proj.weight')] = k_weight
            ckpt[key.replace('qkv.weight', 'v_proj.weight')] = v_weight
        elif 'qkv.bias' in key:
            qkv_bias = ckpt.pop(key)
            q_bias = qkv_bias[:768]
            k_bias = qkv_bias[768:768*2]
            v_bias = qkv_bias[768*2:]
            ckpt[key.replace('qkv.bias', 'q_proj.bias')] = q_bias
            ckpt[key.replace('qkv.bias', 'k_proj.bias')] = k_bias
            ckpt[key.replace('qkv.bias', 'v_proj.bias')] = v_bias
    # second, modify the mlp.fc.weight to match fc.weight
    for key in list(ckpt.keys()):
        if 'mlp.fc' in key:
            fc_weight = ckpt.pop(key)
            ckpt[key.replace('mlp.', '')] = fc_weight
...
def vit_base_patch16_224_adapter_ibot(pretrained=False, **kwargs):
    ...
    #checkpoint_model=timm.create_model("vit_base_patch16_224", pretrained=True, num_classes=0)
    #state_dict = torch.load('./checkpoints/dino_vitbase16_pretrain.pth', map_location='cpu')['state_dict']
    ckpt = torch.load('/root/autodl-tmp/jww/CVPR/ACIL-LAMDA-PILOT/checkpoints/ibot.pth', map_location='cpu')['state_dict']
    #state_dict = checkpoint_model.state_dict()
    # modify the checkpoint state dict to match the model
    # first, split qkv weight into q, k, v
    for key in list(ckpt.keys()):
